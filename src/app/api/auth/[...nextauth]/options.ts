...
export const options:any= {
    providers:[
        ...
        CredentialsProvider({
            name:"credentials",
            credentials:{
                email:{label:"Email", type:"email",placeholder:"Enter your email address",required:true},
            ...
            async authorize(credentials) {
               try {
                ...
                const comparePass = await bcrypt.compare(credentials?.password!, userExist.password );
                if(!comparePass){
                    return NextResponse.json({message:"email or password is incorrect"});
                }
