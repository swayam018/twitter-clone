...
function ProfileFeed({ userdata }: any) {
    ...
    return (
        <div className=' text-white bg-black w-[598px] border-r border-gray-500 pt-5  relative '>
            <nav className=' flex w-full border-b border-gray-500 sticky top-[53px] bg-black/80  backdrop-blur-sm'>
            ...
            {tabValue === "likes" && showTabValue.current.length > 0 && (
                <>
                    {showTabValue.current.map((post: any) => {
                        return <React.Fragment key={post._id}>
                            <Posts post={post} />
